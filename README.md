...
`site-local.example.ps1` lists every knob - paths, log directory, timestamp,
extra redactions, and whether to pause - with an example for each.
...
snapshot provides, so they drop straight in. To rebuild from source instead,
fetch the cygport and patches with `bin/resolve-pinned-deps.sh --no-dry-run`
(see Pinned dependencies, below).

`DEPENDENCIES.lock` records what makes those two packages reproducible. Both
now live in their own repos,
[phdye-cygwin/postfix](https://github.com/phdye-cygwin/postfix) and
[phdye-cygwin/heirloom-mailx](https://github.com/phdye-cygwin/heirloom-mailx),
pinned here by commit; `cygport/` no longer holds their source. Run
`bin/resolve-pinned-deps.sh` to check the pin, or `--no-dry-run` to fetch a
package's source into `cygport/<name>` for a local rebuild (it verifies the
fetched commit matches the pin before calling it done). The built `.tar.xz`
packages installers actually consume stay vendored in `packages/`, unaffected
by this split.
...
bin/resolve-pinned-deps.sh    verify, or --no-dry-run to fetch, the pins in DEPENDENCIES.lock
...
cygport/                      empty by default; populated by resolve-pinned-deps.sh --no-dry-run
